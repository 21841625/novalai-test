#!/usr/bin/env python3
"""
Deep manuscript review via Opus (or configured review model).

Usage:
  python review.py              # Generate review and parse it
  python review.py --output reviews.md  # Save human-readable review
...
def call_opus(prompt, max_tokens=8000):
    ...
    return call_llm(prompt, REVIEW_MODEL, max_tokens, temperature=0.3)
...
def should_stop(parsed_review):
    ...
    if stars >= 4 and total > 0 and qualified / total > 0.5:
        return True, f"\u2605\u2605\u2605\u2605 {stars} with {qualified}/{total} items qualified"
    if total <= 2:
